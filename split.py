# -*- coding: utf-8 -*-
"""
Created on Tue Nov 24 14:31:22 2020
...
from tkinter import Toplevel,LabelFrame,Label,Entry,W,ttk
...
class spliting():
    def __init__(self,mainframe,project_info):
        ...
        self.project_path = os.path.split(project_info[project_info['模块类型']=='project']['保存地址'][0])[0]  ##获取project存储的地址
         
        self.node_name='split'
        self.par_train_data=df()
        self.method=''
        self.replace='False'
        self.seed=123456
        self.train_pct=0.7
        ...
        self.load='N'
        self.save='N'
        self.exist_data = list(project_info['模块名字'])  ##防止名字重复
    ...
    def ui_start(self):
        ...
        train_address = self.project_info[((self.project_info["模块类型"]=='DATA')|(self.project_info['模块类型']=='SAMPLE'))
                        &(self.project_info["状态"]=='Good')]["保存地址"]
        train_data_list = []  ##获取可以进行分区的数据集的名称
        for add in train_address:
            try:
                fr = open(add,'rb')
                node_info = pickle.load(fr)
                fr.close()
                data_role=node_info[0]['data_role']  ##0是node_setting
        ...
        L00.grid(row=0, column=0, sticky=(W))
        if self.load=='N':
        ...
        if self.load=='Y':
            for i in range(len(train_data_list)):
                if train_data_list[i]==self.previous_node_name:  ##上一次分区的数据集的名称
        ...
        if self.load=='Y':
            ...
            self.comboxlist_method.current(0)
        self.comboxlist_method.grid(row=2, column=1, sticky=(W))
        ...
        if self.load=='Y':
            ...
            self.comboxlist_replace.current(0)
        self.comboxlist_replace.grid(row=3, column=1, sticky=(W))
    ...
    def load_data(self,event,datatype):
        ...
        try:
            ...
            if (datatype=='train')&(len(self.comboxlist_train_data.get())>=1):
                ...
                self.par_train_data=node_info[1]  ##没有分区的全部的数据
    ...
    def get_par(self,event):
        #更新得到的设置
        self.load_data(event,'train')  ##在前面界面的时候不是筛选过一次了吗？
    ...
    def split_function(self):
        ...
        if self.sample_flag=='是':
            ...
            good_num=round((bad_num/self.bad_rate)*(1-self.bad_rate))
    ...
    def result_ui(self,mainframe,ac): ##进行结果展示
        ...
        target = self.par_traindatavariable_setting[self.par_traindatavariable_setting["变量角色"]=='目标'][0]
        target=list(self.par_traindatavariable_setting[ self.par_traindatavariable_setting['变量角色']=='目标']['变量名称'])[0]
        ...
        i = 0
        for da in data_list:
            tree.insert('', i, values = (da.get("data"), da.get("obs"), da.get("bad_num"), da.get("bad_rate")))
            i +=1 
        tree.grid()
